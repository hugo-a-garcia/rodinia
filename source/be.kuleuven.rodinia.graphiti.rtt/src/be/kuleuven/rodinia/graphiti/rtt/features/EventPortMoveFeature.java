...
public class EventPortMoveFeature extends DefaultMoveShapeFeature{

	@SuppressWarnings("unused")
	private boolean left = false;
	
	public EventPortMoveFeature(IFeatureProvider fp) {
		super(fp);
	}
	...
	protected void postMoveShape(IMoveShapeContext context) {
	    ...
	    if (containerShape.getGraphicsAlgorithm().getX() > (context.getTargetContainer().getGraphicsAlgorithm().getWidth() /2) - (context.getTargetContainer().getGraphicsAlgorithm().getWidth()/10)){
	        ...
	        int lengtWithText = containerWidth - TextUtils.calculateTextWidth(((EventPort) getBusinessObjectForPictogramElement(containerShape)).getName());
	        if (lengtWithText < 0){
	        	lengtWithText = 0;
	        }
