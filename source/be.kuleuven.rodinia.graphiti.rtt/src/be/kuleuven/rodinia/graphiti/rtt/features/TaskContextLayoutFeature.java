...
public class TaskContextLayoutFeature extends AbstractLayoutFeature {
	...
	public boolean canLayout(ILayoutContext context) {
		...
		return businessObjects.size() == 1 && businessObjects.get(0) instanceof TaskContext;
	}
		...
		for (Shape shape : containerShape.getChildren()) {
			...
			if (containerWidth != size.getWidth()) {
				if (graphicsAlgorithm instanceof Polyline) {
					...
					Point newSecondPoint = gaService.createPoint(containerWidth - 10, secondPoint.getY());
					...
					if (graphicsAlgorithm instanceof Rectangle) {
						...
						if (!(graphicsAlgorithm instanceof RoundedRectangle)) {
							if (graphicsAlgorithm.getX() > 15) {
								gaService.setLocation(graphicsAlgorithm, (containerWidth - width) - 10,
							...
							if (graphicsAlgorithm instanceof RoundedRectangle) {
								if (size.getHeight() == 25) {
									...
									if (graphicsAlgorithm.getX() == 0) {
										...
										gaService.setLocation(graphicsAlgorithm, containerWidth - size.getWidth(),
												graphicsAlgorithm.getY());
									}
								} else {
									if (size.getHeight() == 24) {
										gaService
												.setLocationAndSize(graphicsAlgorithm, 10, 19, containerWidth - 50, 24);
									} else {
										if (size.getHeight() == 23) {
											final Object container = getBusinessObjectForPictogramElement(context
													.getPictogramElement());
											final Object target = getBusinessObjectForPictogramElement(graphicsAlgorithm
													.getPictogramElement());
											int number = 0;
													...
													for (Property propx : task.getProperties()) {
														if (propx.getName().equals(prop.getName())) {
															...
															if ((19 + (number + 1) * 23) >= 42 + ((containerHeight - 42) / 2)) {
																...
																for (GraphicsAlgorithm graph : graphicsAlgorithm
